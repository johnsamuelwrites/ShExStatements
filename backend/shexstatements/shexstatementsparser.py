...
class ShExStatementLexerParser:
    ...
    def buildparser(self, **kwargs):
        ...
        if 'outputdir' not in kwargs:
            kwargs['outputdir'] = _ply_output_dir
        # Suppress debug/error logs unless explicitly set
        if 'debuglog' not in kwargs:
            kwargs['debuglog'] = None
        if 'errorlog' not in kwargs:
            kwargs['errorlog'] = None
        self.lexer = lex.lex(module=self, outputdir=kwargs.get('outputdir'))
